...
namespace Keycloak.Net.Common.Extensions
{
    public static class FlurlRequestExtensions
    {
        private static async Task<string> GetAccessTokenAsync(string url, string realm, string userName, string password)
        {
            var result = await url
                ...
                .PostUrlEncodedAsync(new List<KeyValuePair<string, string>>
                {
                    ...
                    new KeyValuePair<string, string>("client_id", "karnel-travel-client")
                })
		...
		//private static async Task<string> GetAccessTokenOfSpecificClientAsync(string url, string realm, string clientSecret, string clientId)
		//{
		//	var result = await url
		//		.AppendPathSegment($"/realms/{realm}/protocol/openid-connect/token")
		//		.WithHeader("Content-Type", "application/x-www-form-urlencoded")
		...
		//private static string GetAccessTokenOfSpecificClient(string url, string realm, string clientSecret, string clientId) => GetAccessTokenOfSpecificClientAsync(url, realm, clientSecret, clientId).GetAwaiter().GetResult();
