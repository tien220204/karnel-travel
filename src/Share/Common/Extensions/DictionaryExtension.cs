...
public static class DictionaryExtension
{
    ...
    /// <summary>
    /// Build log content from a dictionary
    /// </summary>
    /// <typeparam name="TKey">The type of the key.</typeparam>
    /// <typeparam name="TValue">The type of the value.</typeparam>
    /// <param name="dictionary">dictionary</param>
    /// <returns></returns>
    public static string ToLogContent<TKey, TValue>(this IDictionary<TKey, TValue> dictionary)
    {
        if (dictionary != null)
        {
            ...
            {
                contentBuilder.Append($"(Key: {p.Key}, Value: {p.Value}); ");
            }
    ...
    /// <summary>
    /// Convert an object of a class to dictionary for logging, all keys are object's properties
    /// </summary>
    /// <param name="obj">the object which need to be converted</param>
    /// <param name="ignoredProperties">ignored properties</param>
    /// <returns></returns>
    public static IDictionary<string, object> ToLogDictionary<T>(this T obj, params string[] ignoredProperties) where T : class
    {
        var dict = new Dictionary<string, object>();
        var finalIgnoredProperties = ignoredProperties ?? Array.Empty<string>();
        if (obj != null)
        {
            var objGetProperties = obj.GetType().GetProperties();
            foreach (var p in objGetProperties)
            {
                if (p != null && p.GetMethod != null && !finalIgnoredProperties.Any(ip => ip == p.Name))
                {
